	...
	$.weatherSlider = function(el, options) {
				...
				if( data['current_condition'] ){
					...
					$.getJSON(url, function(d) {
						...
						if( wIcon.indexOf('_night') != -1 || ( wCode != 122 && wIcon.indexOf('_black') != -1 ) ){
							...
							if( ( parseInt(curTime[0]) < ws.o.daytime[0] || parseInt(curTime[0]) > ws.o.daytime[1]-1 ) ){
								wData['current_condition']['daytime'] = 'night';								
							}else{
		...
		ws.prevNext = function(id){
			...
			if( ws.g.locations.length > 1 ){
				...
				var ptext = ws.g.locations[prev].split(',')[0] == '' ? ws.l.cl : ws.g.locations[prev].split(',')[0];
				var ntext = ws.g.locations[next].split(',')[0] == '' ? ws.l.cl : ws.g.locations[next].split(',')[0];
